...
public class FileEncryptUtil {
    ...
    public static byte[] encrypt(RandomAccessFile in, RandomAccessFile out, byte[] AESkey,
                                 ...
                                 List<PublicKey> publicKeys) throws
            IOException,
            InvalidCipherTextException,
            NoSuchAlgorithmException,
            NoSuchPaddingException,
        ...
        for (int i = 0; i < usernames.size(); i++) {
            byte[] name = PasswordUtil.hash(NameKey.xor(filehash, PasswordUtil.hash(usernames.get(i).getBytes())));
            byte[] key = RSAEncryptUtil.encrypt(Base64.encodeBase64String(AESkey).getBytes(), publicKeys.get(i));
            header.addNameKey(new NameKey(name, key));
        }
    ...
    public static void decrypt(RandomAccessFile in, RandomAccessFile out,
                               String username, PrivateKey privateKey) throws
            IOException,
            InvalidCipherTextException,
            NoSuchAlgorithmException,
            NoSuchProviderException,
        ...
        byte[] key = Base64.decodeBase64(RSAEncryptUtil.decrypt(nameKey.getKey(), privateKey));
